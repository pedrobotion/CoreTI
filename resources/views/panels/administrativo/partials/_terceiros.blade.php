<section class="rounded-lg border-2 border-rose-200 bg-rose-50 p-5 shadow-sm">
    <div class="mb-3 flex items-center justify-between">
        ...
        <span class="rounded-md bg-rose-200 px-2 py-1 text-xs font-semibold text-rose-900">{{ $pendingThirdParty->count() }} pendências</span>
    </div>
    ...
    <div class="overflow-x-auto">
        <table class="w-full min-w-[980px] text-sm">
            ...
            <tbody class="divide-y divide-slate-200">
                @forelse($pendingThirdParty as $item)
                    @php
                    ...
                    <tr x-data="{ modalOpen: false }">
                        ...
                        <td class="px-3 py-2">
                            <div class="inline-flex flex-col items-start gap-0.5">
                                <span class="inline-flex rounded-md px-2 py-0.5 text-[11px] font-semibold {{ $infoStatusClass }}">{{ $infoStatusLabel }}</span>
                                <span class="text-[10px] font-medium text-slate-500">{{ $infoStatusDescription }}</span>
                            </div>
                        </td>
                        <td class="px-3 py-2">{{ $item->unidade_setor }}</td>
                        <td class="px-3 py-2">
                            <button type="button" @click.stop="modalOpen = true" class="inline-flex min-h-10 items-center justify-center rounded-md border border-[#033151] bg-[#033151] px-3 py-2 text-xs font-semibold text-white shadow-sm transition hover:opacity-90">{{ $actionLabel }}</button>
                                ...
                                >
                                    <div class="w-full max-w-3xl max-h-[calc(100vh-1.5rem)] overflow-hidden rounded-lg bg-white shadow-2xl sm:max-h-[calc(100vh-2rem)]">
                                        <div class="flex items-center justify-between border-b border-slate-200 px-5 py-4">
                                            <h3 class="text-lg font-bold text-slate-900">Processo administrativo | {{ $item->plaqueta }}</h3>
                                        ...
                                        <div class="max-h-[calc(100vh-8rem)] overflow-y-auto">
                                            @if($thirdPartyStage === 'aguardando_envio')
                                                <form
                                                    x-data="{ selectedCompanyCnpj: @js($item->terceiros_cnpj ?? '') }"
                                                    ...
                                                    action="{{ route('bancada-servicos.assets.administrative.process', $item) }}"
                                                ...
                                                >
                                                    ...
                                                    <div class="sm:col-span-2">
                                                        ...
                                                        <textarea rows="2" readonly class="mt-1 w-full rounded-md border-slate-300 bg-slate-100 text-sm text-slate-700">{{ $item->terceiros_problema ?: '-' }}</textarea>
                                                    </div>
                                                    <div>
                                                        ...
                                                        >
                                                            ...
                                                            @foreach(($thirdPartyCompanies ?? collect()) as $company)
                                                                <option value="{{ $company->name }}" data-cnpj="{{ $company->cnpj }}" @selected($item->terceiros_empresa === $company->name)>{{ $company->name }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                    <div>
                                                    ...
                                                    <div>
                                                        ...
                                                        <input name="terceiros_nota_remessa" value="{{ $item->terceiros_nota_remessa }}" class="mt-1 w-full rounded-md border-slate-300 text-sm">
                                                    </div>
                                                    <div class="sm:col-span-2">
                                                        <label class="text-sm font-medium">Observações</label>
                                                        <textarea name="terceiros_observacoes" rows="2" class="mt-1 w-full rounded-md border-slate-300 text-sm">{{ $item->terceiros_observacoes }}</textarea>
                                                    </div>
                                                    <div class="sm:col-span-2">
                                            ...
                                            @elseif($thirdPartyStage === 'aguardando_informacoes')
                                                <div class="grid gap-4 px-5 py-4">
                                                    <div class="grid gap-3 rounded-md border border-slate-200 bg-slate-50 p-4 sm:grid-cols-2">
                                                        ...
                                                        <div>
                                                            ...
                                                            <div class="mt-1 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">{{ $item->plaqueta }}</div>
                                                        </div>
                                                        <div>
                                                            <label class="text-xs font-semibold uppercase text-slate-500">Tipo</label>
                                                            <div class="mt-1 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">{{ $item->tipo_equipamento ?: '-' }}</div>
                                                        </div>
                                                        <div class="sm:col-span-2">
                                                            <label class="text-xs font-semibold uppercase text-slate-500">Problema</label>
                                                            <div class="mt-1 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 whitespace-pre-wrap">{{ $item->terceiros_problema ?: '-' }}</div>
                                                        </div>
                                                        <div>
                                                            <label class="text-xs font-semibold uppercase text-slate-500">Empresa</label>
                                                            <div class="mt-1 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">{{ $item->terceiros_empresa ?: '-' }}</div>
                                                        </div>
                                                        <div>
                                                            <label class="text-xs font-semibold uppercase text-slate-500">CNPJ</label>
                                                            <div class="mt-1 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">{{ $item->terceiros_cnpj ?: '-' }}</div>
                                                        </div>
                                                        <div>
                                                            <label class="text-xs font-semibold uppercase text-slate-500">Nota remessa</label>
                                                            <div class="mt-1 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">{{ $item->terceiros_nota_remessa ?: '-' }}</div>
                                                        </div>
                                                        <div>
                                                            <label class="text-xs font-semibold uppercase text-slate-500">OS</label>
                                                            <div class="mt-1 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700">{{ $item->terceiros_os_numero ?: '-' }}</div>
                                                        </div>
                                                        <div class="sm:col-span-2">
                                                            <label class="text-xs font-semibold uppercase text-slate-500">Observações</label>
                                                            <div class="mt-1 rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-700 whitespace-pre-wrap">{{ $item->terceiros_observacoes ?: '-' }}</div>
                                                        </div>
                                                        <div class="sm:col-span-2">
                                                            @if($remessaAttachment)
                                                                <a
                                                                    href="{{ route('bancada-servicos.attachments.download', $remessaAttachment) }}"
                                                                    target="_blank"
                                                    ...
                                                    <div class="rounded-md border border-slate-200 bg-white p-4" x-data="{
                                                        informacaoRecebida: @js(in_array((string) $item->terceiros_resultado, ['sem_conserto', 'negada', 'reprovada', 'reprovado'], true) ? 'sem_conserto' : ''),
                                                        showValor() {
                                                        ...
                                                        <form x-cloak x-show="informacaoRecebida" method="POST" enctype="multipart/form-data" action="{{ route('bancada-servicos.assets.administrative.process', $item) }}" class="mt-4 grid gap-3 rounded-md border border-slate-200 bg-slate-50 p-4 sm:grid-cols-2">
                                                            @csrf
                                                            @method('PATCH')
                                                            ...
                                                            <div>
                                                                ...
                                                                <input name="terceiros_os_numero" value="{{ $item->terceiros_os_numero }}" required class="mt-1 w-full rounded-md border-slate-300 text-sm">
                                                            </div>
                                                            <div x-show="showValor()">
                                                                <label class="text-sm font-medium">Valor do reparo / orçamento</label>
                                                                <input name="terceiros_valor_reparo" placeholder="0,00" value="{{ $item->terceiros_valor_reparo !== null ? number_format((float) $item->terceiros_valor_reparo, 2, ',', '.') : '' }}" required class="mt-1 w-full rounded-md border-slate-300 text-sm">
                                                            </div>
                                                            <div class="sm:col-span-2">
                                            ...
                                            @elseif($thirdPartyStage === 'aguardando_retorno_fisico')
                                                <div class="grid gap-4 px-5 py-4">
                                                    ...
                                                    <div class="rounded-md border border-slate-200 bg-white p-4">
                                                        ...
                                                        <form method="POST" action="{{ route('bancada-servicos.assets.administrative.process', $item) }}" class="mt-4 grid gap-3 sm:grid-cols-2">
                                                            @csrf
                                                            @method('PATCH')
                                                            <input type="hidden" name="action" value="terceiros_retorno_fisico">
                                                            <div>
                                                                <label class="text-sm font-medium">Plaqueta</label>
                                                                <div class="mt-1 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">{{ $item->plaqueta }}</div>
                                                            </div>
                                                            <div>
                                                                <label class="text-sm font-medium">Empresa</label>
                                                                <div class="mt-1 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">{{ $item->terceiros_empresa ?: '-' }}</div>
                                                            </div>
                                                            <div>
                                                                ...
                                                                {{ in_array((string) $item->terceiros_resultado, ['aprovada', 'aprovado'], true) ? 'Orçamento realizado' : (in_array((string) $item->terceiros_resultado, ['sem_conserto', 'negada', 'reprovada', 'reprovado'], true) ? 'Sem conserto' : 'Não informado') }}
                                                                </div>
                                                            </div>
                                                            <div>
                                                                <label class="text-sm font-medium">OS da empresa</label>
                                                                <div class="mt-1 rounded-md border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">{{ $item->terceiros_os_numero ?: '-' }}</div>
                                                            </div>
                                                            <div class="sm:col-span-2">
