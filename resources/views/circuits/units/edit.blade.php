        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg border border-gray-100">
                <div class="p-6 text-gray-900">
                    <form method="POST" action="{{ route('circuits.units.update', $unit) }}" class="space-y-4"
                        x-data="{
                    ...
                    >
                        ...
                        <div>
                            ...
                            <select name="operadora" class="mt-1 block w-full rounded-md border-gray-200" required>
                                ...
                                @foreach ($operadorasCadastro as $operadora)
                                    <option value="{{ $operadora }}" @selected(old('operadora', $unit->operadora) === $operadora)>{{ $operadora }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="id_unidades" x-ref="unitSelect" @change="fillUfFromUnit" class="mt-1 block w-full rounded-md border-gray-200" required>
                                ...
                                @foreach ($unidades as $unidade)
                                    <option value="{{ $unidade->id_unidades }}" @selected(old('id_unidades', $unit->id_unidades) == $unidade->id_unidades)>
                                        {{ $unidade->unidade }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <input type="text" name="servico" value="{{ old('servico', $unit->servico) }}" class="mt-1 block w-full rounded-md border-gray-200" required />
                            @error('servico')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Endereço</label>
                            <input type="text" name="endereco" x-ref="enderecoInput" value="{{ old('endereco', $unit->endereco) }}" class="mt-1 block w-full rounded-md border-gray-200" required />
                        ...
                        <div>
                            ...
                            <input type="text" name="contato" value="{{ old('contato', $unit->contato) }}" class="mt-1 block w-full rounded-md border-gray-200" required />
                            @error('contato')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                        </div>

                        <label class="inline-flex items-center gap-2 text-sm text-gray-700">
                            <input type="checkbox" name="contato_whatsapp" value="1" @checked(old('contato_whatsapp', $unit->contato_whatsapp)) class="rounded border-gray-300 text-slate-900 focus:ring-slate-900">
                            Contato via WhatsApp
                        </label>

                        <div>
                            <label class="block text-sm font-medium text-gray-700">Informações Adicionais</label>
                            <textarea name="informacoes_adicionais" rows="4" class="mt-1 block w-full rounded-md border-gray-200">{{ old('informacoes_adicionais', $unit->informacoes_adicionais) }}</textarea>
